...
describe('Huffman', () => {
  ...
  it('should satisfy the entropy condition (H <= cost <= H+1)', () => {
    messages.forEach(message => {
      ...
      const entropy = Object.keys(frequencies).reduce((partial, char) => {
        const freq = frequencies[char];
        return partial - freq * Math.log(freq);
      }, 0) / Math.log(2);
      const encoding = huffman.encode(message).encoding;
      const cost = Object.keys(encoding).reduce((partial, char) => partial + frequencies[char] * encoding[char].length, 0);
      assert(entropy <= cost);
