...
describe('In-polygon detection', function(){
    it('should be able to distinguish whether a point is inside a polygon', function(){
        var polygon = [ new Vector2D(1, 1), new Vector2D(1, 2), new Vector2D(2, 2), new Vector2D(2, 1) ];
        assert(inPolygon(polygon, new Vector2D(1.5, 1.5)));
        assert(!inPolygon(polygon, new Vector2D(5, 1)));
    })
    it('should be able to treat on-boundary points as inside', function(){
        var polygon = [ new Vector2D(1, 1), new Vector2D(1, 2), new Vector2D(2, 2), new Vector2D(2, 1) ];
